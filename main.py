...
BASE_WEBHOOK_URL = os.getenv("BASE_WEBHOOK_URL")  # ví dụ: https://your-app.koyeb.app
...
if not DATABASE_URL:
    raise RuntimeError("Thiếu biến môi trường DATABASE_URL (Neon connection string).")
...
class Database:
    ...
    def _create_tables(self):
        ...
        cur.execute(
            ...
            CREATE TABLE IF NOT EXISTS budgets (
                ...
                essential DOUBLE PRECISION,
                long_term DOUBLE PRECISION,
    ...
    def get_or_create_user(self, telegram_id: int, full_name: str | None) -> int:
        ...
        now = datetime.utcnow()
        cur.execute(
            "INSERT INTO users (telegram_id, full_name, created_at) VALUES (%s, %s, %s) RETURNING id",
    ...
    def set_limit(self, user_id: int, category: str, period: str, limit_amount: float):
        ...
        if row:
            cur.execute(
                "UPDATE limits SET limit_amount=%s WHERE id=%s",
                (limit_amount, row["id"]),
            )
        else:
            cur.execute(
...
async def salary_enter_amount(message: Message, state: FSMContext):
    ...
    if not all([essential_w, long_w, invest_w, personal_w]):
        await message.answer(
            "❌ Không tìm thấy đủ 4 ví mặc định. Bạn hãy kiểm tra lại cấu hình ví.",
...
def _build_csv_from_rows(rows):
    ...
    for r in rows:
        tx_id = r["id"]
        dt = r["created_at"].isoformat() if r["created_at"] else ""
        tx_type = r["type"] or ""
...
async def cmd_export(message: Message):
    ...
    csv_bytes = _build_csv_from_rows(rows)
...
# ---------- /backup – backup dữ liệu user (CSV) ----------
...
async def cmd_backup(message: Message):
    ...
    if not rows:
        await message.answer(
            "📦 Bạn chưa có giao dịch nào để backup.",
            reply_markup=main_menu_kb(),
        )
        return

    csv_bytes = _build_csv_from_rows(rows)
    filename = f"backup_finance_user_{message.from_user.id}.csv"
    buf = BufferedInputFile(
        csv_bytes,
        filename=filename,
    )
...
def create_app() -> web.Application:
    ...
    async def health(request: web.Request):
        return web.Response(text="OK - finance bot (Koyeb + Neon) is running")
